...
def get_articles_with_sources_details(
    session: Session,
) -> list[Article]:
    statement = (
        select(Article)
...
def show_articles_with_sources(
    session: Session,
):
    articles = get_articles_with_sources(session)
    for article in articles:
        print("+", article)
...
def show_articles_with_sources_and_details(
    session: Session,
):
    articles = get_articles_with_sources_details(session)
    for article in articles:
        print("+", article)
...
def show_sources_with_articles(
    session: Session,
):
    sources = get_sources_with_articles(session)
...
def main():
    with Session(engine) as session:
        ...
        # create_sources(session)
        # create_new_articles(session)
        ...
        #     print("---")
        # auto_associate_articles_with_plain_sources(session)
        # show_articles_with_sources(session)
        # show_sources_with_articles(session)
        show_articles_with_sources_and_details(session)
