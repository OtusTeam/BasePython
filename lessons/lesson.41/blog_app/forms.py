        ...
        if content or title:
            for word in forbidden_words:
                if word in content.lower():
                    raise ValidationError(f'Контент не должен содержать запрещенное слово {word}')
                if word in title.lower():
                    raise ValidationError(f'Заголовок не должен содержать запрещенное слово {word}')
