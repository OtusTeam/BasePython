...
def upgrade() -> None:
    op.add_column("author", sa.Column("promocode", sa.String(), nullable=True))
    op.create_unique_constraint(op.f("uq_author_promocode"), "author", ["promocode"])
    table = sa.Table(
        "author",
        sa.MetaData(),
...
def downgrade() -> None:
    op.drop_constraint(op.f("uq_author_promocode"), "author", type_="unique")
