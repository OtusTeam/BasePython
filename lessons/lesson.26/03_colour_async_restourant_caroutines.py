...
async def main():
    ...
    # Создание списка задач (корутин) для каждого заказа
    tasks = [asyncio.create_task(handle_order(order, cooking_time)) for order, cooking_time in orders.items()]
    # Обратите внимание на ^ create_task, именно это отличает от gather
    
    for task in tasks:
        await task  # Явно ожидаем завершения каждой задачи
