...
async def find_author_by_user_username(session: AsyncSession, user_name: str) -> Author | None:
...
async def find_posts_by_tags_names(session: AsyncSession, *tags_names: str) -> list[Post]:
    posts_stmt = (
        select(Post)
        .join(Post.tags)
        # .join(Tag, Post.tags)
...
async def main():
    async with async_session() as session:
        ...
        # await create_posts(session, author_sam, "Python Django Lesson", "Python Flask Lesson")
        #
